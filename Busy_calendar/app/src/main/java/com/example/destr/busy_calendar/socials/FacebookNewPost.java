...
public class FacebookNewPost {
    ...
    public void getResponse(Context mContext, final String status){
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                mMyHttpClient.method(String.format(Constants.UrlConstants.FACEBOOK_POST_SET,status,facebookToken),Constants.OtherConstants.POST_METHOD);
        }
    }).start();
}
}
